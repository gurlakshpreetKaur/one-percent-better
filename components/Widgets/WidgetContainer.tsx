...
export default function WidgetContainer({ children, widgetName, noPadding = false }: { children: JSX.Element | JSX.Element[], widgetName: string, noPadding?: boolean }) {
    return <div className="bg-yerba-mate-300 rounded-xl px-5 pt-4 pb-5 group flex flex-col h-[50vh]">
        <header className="inline-block mb-3 hover:cursor-pointer w-full">
            <h3 className="font-source text-yerba-mate-700  group-hover:text-white font-semibold text-xl inline-block">{widgetName}</h3>
        </header>
